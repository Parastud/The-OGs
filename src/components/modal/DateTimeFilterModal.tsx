...
function Chip({ icon, label, isActive, accentColor, onPress }: ChipProps) {
  return (
    ...
    >
      ...
      <Text
        style={[
          SEMI_BOLD_TEXT(12, isActive ? accentColor : COLORS.textPrimary),
          { flex: 1 },
        ]}
        numberOfLines={1}
      >
...
export default function DateTimeFilterModal({
  ...
  const [startDate, setStartDate] = useState<Date>(
    isCustom ? (selectedFilter as any).startDate : new Date(),
  );
  const [startTime, setStartTime] = useState<Date>(
    isCustom ? (selectedFilter as any).startTime : midnight(),
  );
  const [endDate, setEndDate] = useState<Date>(
    isCustom ? (selectedFilter as any).endDate : new Date(),
  );
  const [endTime, setEndTime] = useState<Date>(
    isCustom ? (selectedFilter as any).endTime : new Date(),
  );
  ...
  return (
    ...
    >
      <Pressable style={styles.backdrop} onPress={onCancel}>
        <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
          <View style={styles.header}>
            <Text style={BOLD_TEXT(16, COLORS.textPrimary)}>
              Filter by Date & Time
            </Text>
            <TouchableOpacity
              onPress={onCancel}
          ...
          <View style={styles.modeToggle}>
            {(["preset", "custom"] as const).map((mode) => (
              ...
              >
                <Text
                  style={SEMI_BOLD_TEXT(
                    13,
                    activeMode === mode ? COLORS.white : COLORS.textSecondary,
                  )}
                >
                  ...
                  return (
                    ...
                    >
                      <Text
                        style={SEMI_BOLD_TEXT(
                          14,
                          sel ? COLORS.primary : COLORS.textPrimary,
                        )}
                      >
                        {item.label}
                      </Text>
            ...
            {activeMode === "custom" && (
              <View style={styles.rangeCard}>
                <View style={styles.rail}>
                  <View
                    style={[styles.railDot, { backgroundColor: START_COLOR }]}
                  />
                  ...
                  <View
                    style={[
                      styles.railDot,
                      {
                        backgroundColor: rangeError ? COLORS.error : END_COLOR,
                      },
                    ]}
                  />
                </View>
                ...
                <View style={styles.rangeContent}>
                  ...
                  <View style={styles.rangeSection}>
                    <Text
                      style={SEMI_BOLD_TEXT(
                        11,
                        rangeError ? COLORS.error : END_COLOR,
                      )}
                    >
                      TO
                    </Text>
                    ...
                    {rangeError && (
                      <View style={styles.errorBox}>
                        <Text style={SEMI_BOLD_TEXT(11, COLORS.error)}>
                          {rangeError}
                        </Text>
                      </View>
                    )}
            ...
            {activeMode === "custom" && !rangeError && (
              <View style={styles.summaryStrip}>
                <View style={styles.summaryItem}>
                  ...
                  <Text style={SEMI_BOLD_TEXT(12, COLORS.textPrimary)}>
                    {formatDate(startDate)}
                  </Text>
                  <Text style={SEMI_BOLD_TEXT(11, START_COLOR)}>
                ...
                <View style={styles.summaryItem}>
                  ...
                  <Text style={SEMI_BOLD_TEXT(11, END_COLOR)}>
                    {formatTime(endTime)}
                  </Text>
                </View>
              </View>
      ...
      {Platform.OS === "android" && androidTarget !== null && (
        <DateTimePicker
          value={
            androidTarget === "start-date"
              ? startDate
              : androidTarget === "start-time"
                ? startTime
                : androidTarget === "end-date"
                  ? endDate
                  : endTime
          }
