...
export const EstimatedBillCard = ({
  ...
  const { elm_dg_forecasted_unit, elm_grid_forecasted_unit } = dashboardSummaryData;

  const dgUnitPrice = parseFloat(deviceEntries.find(e => e.type === '2')?.unit_price ?? '0');
  const gridUnitPrice = parseFloat(deviceEntries.find(e => e.type === '3')?.unit_price ?? '0');
